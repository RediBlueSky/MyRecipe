...
if status in login_user:
   ...
   if login in users and users[login] == passw:
     ...
     while not (login == 'guest' and passw == 'guest'):
            print("\nUser doesn't exist or wrong password!\n")
            print('\nPlease enter as a Guest, for login name: "guest"  passw: "guest" ')
...
if search in c_m_t and status in all_user :
    for key in data:
        ...
        for each in category_meal_type:
            if search in each:
                result = key
                print('\n'+ result)
elif search in name_of_food and status in all_user:
     result = search.title()
     print(result)
...
if (asd.isnumeric() and asd == '3') and status in all_user:
    ...
    for i in direct:
     ...
     print('Ingredients: ', *[x for each in ingr for x in each], sep=', ')
     print('Nutritious: ', nutr, sep= ',' )
